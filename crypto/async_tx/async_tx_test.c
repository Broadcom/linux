...
MODULE_PARM_DESC(thread_count, "Number of threads, Should be atleast 1");

static unsigned int disk_block_size = PAGE_SIZE;
module_param(disk_block_size, uint, 0644);
MODULE_PARM_DESC(disk_block_size,
		 "Size of disk block, between 8 to PAGE_SIZE and multiple of 8");
...
MODULE_PARM_DESC(request_count,
		 "Number of request submitted in one iteration, at least 1");

static unsigned int iteration_count = 1;
module_param(iteration_count, uint, 0644);
MODULE_PARM_DESC(iteration_count, "Number of iterations, at least 1");
...
static int async_tx_test_thread_main(void *data)
{
	...
	s64 iter_usecs, min_usecs, max_usecs, avg_usecs;
	unsigned long long iter_kbs, min_kbs, max_kbs, avg_kbs;
