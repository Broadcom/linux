...
int bcm_rdb_phy_config_intr(struct phy_device *phydev)
{
	...
	if (phydev->interrupts == PHY_INTERRUPT_ENABLED) {
		...
		reg &= ~MII_BCM54XX_ECR_IM;
		err = bcm_phy_write_rdb(phydev, BCM_542XX_RDB_PHY_ECR, reg);
	} else {
		reg |= MII_BCM54XX_ECR_IM;
		err = bcm_phy_write_rdb(phydev, BCM_542XX_RDB_PHY_ECR, reg);
		if (err)
		...
		err = bcm_rdb_phy_ack_intr(phydev);
	}
	return err;
}
...
irqreturn_t bcm_rdb_phy_handle_interrupt(struct phy_device *phydev)
{
	...
	irq_status = bcm_phy_read_rdb(phydev, BCM_542XX_RDB_PHY_ISR);
	if (irq_status < 0) {
		phy_error(phydev);
		return IRQ_NONE;
	}
	 ...
	 * sources.
	 */
	irq_mask = bcm_phy_read_rdb(phydev, BCM_542XX_RDB_PHY_IMR);
	if (irq_mask < 0) {
		phy_error(phydev);
		return IRQ_NONE;
	}
...
EXPORT_SYMBOL_GPL(bcm_rdb_phy_handle_interrupt);
