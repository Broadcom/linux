...
#define DTE_REGS_TO_RESTORE	4
#define DTE_REG_WIDTH		4
...
struct ptp_dte {
	...
	u32 reg_val[DTE_REGS_TO_RESTORE];
#endif
};
...
static void dte_write_nco(void __iomem *regs, s64 ns)
{
	...
	/* compensate for ignoring sum1 (if sum2 will not overflow) */
...
static int ptp_dte_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "ptp clk probe success\n");
...
static int ptp_dte_remove(struct platform_device *pdev)
{
	...
	writel(0, ptp_dte->regs + DTE_NCO_INC_REG);
	writel(0, ptp_dte->regs + DTE_NCO_LOW_TIME_REG);
	writel(0, ptp_dte->regs + DTE_NCO_TIME_REG);
	writel(0, ptp_dte->regs + DTE_NCO_OVERFLOW_REG);
...
static int ptp_dte_suspend(struct device *dev)
{
	...
	ptp_dte->reg_val[DTE_NCO_INC_REG] =
		readl(ptp_dte->regs + DTE_NCO_INC_REG);

	/* disable the nco, current state is stored above */
	writel(0, ptp_dte->regs + DTE_NCO_INC_REG);

	ptp_dte->reg_val[DTE_NCO_LOW_TIME_REG] =
		readl(ptp_dte->regs + DTE_NCO_LOW_TIME_REG);
	ptp_dte->reg_val[DTE_NCO_TIME_REG] =
		readl(ptp_dte->regs + DTE_NCO_TIME_REG);
	ptp_dte->reg_val[DTE_NCO_OVERFLOW_REG] =
		readl(ptp_dte->regs + DTE_NCO_OVERFLOW_REG);
...
static int ptp_dte_resume(struct device *dev)
{
	...
	writel(ptp_dte->reg_val[DTE_NCO_LOW_TIME_REG],
		(ptp_dte->regs + DTE_NCO_LOW_TIME_REG));
	writel(ptp_dte->reg_val[DTE_NCO_TIME_REG],
		(ptp_dte->regs + DTE_NCO_TIME_REG));
	writel(((ptp_dte->reg_val[DTE_NCO_OVERFLOW_REG] &
			DTE_NCO_SUM3_MASK) << DTE_NCO_SUM3_WR_SHIFT),
		(ptp_dte->regs + DTE_NCO_OVERFLOW_REG));
	writel(ptp_dte->reg_val[DTE_NCO_INC_REG],
		(ptp_dte->regs + DTE_NCO_INC_REG));
...
MODULE_DESCRIPTION("Broadcom iProc DTE PTP Clock driver");
