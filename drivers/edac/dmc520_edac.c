...
{								\
	...
	mci = data;						\
	edac = mci->pvt_info;					\
	return dmc520_edac_dram_all_isr(irq, mci,		\
...
static int dmc520_edac_probe(struct platform_device *pdev)
{
	...
	if (nintr > ARRAY_SIZE(dmc520_isr_array)) {
		edac_printk(KERN_ERR, EDAC_MOD_NAME,
			    "Invalid device node configuration: # of interrupt config elements (%d) can not exceed %ld.\n",
	...
	for (intr_index = 0; intr_index < nintr; ++intr_index) {
		...
		edac->interrupt_masks[intr_index]) {
			edac_printk(KERN_ERR, EDAC_MC,
				    "interrupt-config error: element %d's interrupt mask %d has overlap.\n",
				    intr_index,
				    edac->interrupt_masks[intr_index]);
			goto err_free_mc;
		}
	...
	if ((edac->interrupt_mask_all | ALL_INT_MASK) != ALL_INT_MASK) {
		edac_printk(KERN_WARNING, EDAC_MOD_NAME,
			    "interrupt-config warning: interrupt mask (0x%x) is not supported by dmc520 (0x%lx).\n",
