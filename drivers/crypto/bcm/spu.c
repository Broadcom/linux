...
void spum_dump_msg_hdr(u8 *buf, unsigned buf_len)
{
	...
	if (spuh->mh.flags & MH_SCTX_PRES) {
		...
		packet_log("IV_OV_OFST:%u EXP_IV_SIZE:%u\n",
	...
	if (spuh->mh.flags & MH_BDESC_PRES) {
		...
		packet_log("    OffsetMAC:%u LengthMAC:%u\n",
			   be16_to_cpu(bdesc->offsetMAC),
			   be16_to_cpu(bdesc->lengthMAC));
		ptr += sizeof(u32);

		packet_log("  BDESC[1] 0x%08x\n", be32_to_cpu(*((u32 *) ptr)));
		packet_log("    OffsetCrypto:%u LengthCrypto:%u\n",
			   be16_to_cpu(bdesc->offsetCrypto),
			   be16_to_cpu(bdesc->lengthCrypto));
		ptr += sizeof(u32);

		packet_log("  BDESC[2] 0x%08x\n", be32_to_cpu(*((u32 *) ptr)));
		packet_log("    OffsetICV:%u OffsetIV:%u\n",
			   be16_to_cpu(bdesc->offsetICV),
			   be16_to_cpu(bdesc->offsetIV));
		ptr += sizeof(u32);
	}
	...
	if (spuh->mh.flags & MH_BD_PRES) {
		...
		packet_log("    Size:%ubytes PrevLength:%u\n",
			   be16_to_cpu(bd->size), be16_to_cpu(bd->PrevLength));
		ptr += 4;
	}
...
/* Given the length of the hash key and encryption key, determine the
 * expected length of a SPU response header.
 */
			...
			unsigned data_size)
{
	...
	if (req_opts->is_aead && req_opts->is_inbound && !req_opts->dtls_aead)
		cipher_len -= hash_parms->digestsize;

	if (req_opts->is_aead) {
		if (req_opts->dtls_aead)
			auth_len = aead_parms->assoc_size + aead_parms->iv_len +
	...
	bdesc->offsetMAC = cpu_to_be16(auth_offset);
			    ...
			    unsigned data_size)
{
	...
	bdesc->lengthCrypto = cpu_to_be16(data_size);
	bdesc->offsetICV = 0;
	bdesc->offsetIV = 0;
